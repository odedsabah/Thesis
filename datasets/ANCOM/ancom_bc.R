...
feature_table_pre_process = function(feature.table, meta.data, sample.var, group.var, 
  ...
  outlier_check <- function(x) {
    ...
    sigma1 = mu2 - mu1 # IQR as proxy for sigma, assuming this is your intent
    sigma2 = sigma1
    pi = 0.75
    n = length(x)
    epsilon = 100
    tol = 1e-5
    score = ifelse(is.na(sigma1) || sigma1 == 0, rep(0, length(x)), 
                   pi * dnorm(x, mean = mu1, sd = sigma1) / ((1 - pi) * dnorm(x, mean = mu2, sd = sigma2)))
    
    while (epsilon > tol && !is.na(sigma1) && sigma1 != 0 && !is.na(sigma2) && sigma2 != 0) {
      grp1.ind = score >= 1
      mu1.new = mean(x[grp1.ind], na.rm = TRUE)
      mu2.new = mean(x[!grp1.ind], na.rm = TRUE)
      sigma1.new = sd(x[grp1.ind], na.rm = TRUE)
      sigma2.new = sd(x[!grp1.ind], na.rm = TRUE)
      pi.new = sum(grp1.ind) / n
      
      # Breaking out of the loop if any new parameter is NA or if sigma1.new or sigma2.new is zero
      if(any(is.na(c(mu1.new, mu2.new, sigma1.new, sigma2.new))) || sigma1.new == 0 || sigma2.new == 0) {
        break
      }
      ...
      epsilon = max(abs(c(mu1 - mu1.new, mu2 - mu2.new, sigma1 - sigma1.new, sigma2 - sigma2.new, pi - pi.new)))
      mu1 = mu1.new; mu2 = mu2.new; sigma1 = sigma1.new; sigma2 = sigma2.new; pi = pi.new
    }
    
    if(!is.na(mu1 + 1.96 * sigma1) && !is.na(mu2 - 1.96 * sigma2) && (mu1 + 1.96 * sigma1 < mu2 - 1.96 * sigma2)) {
      if(pi > 0.85){
        out.ind = (!grp1.ind)
      } else if(pi < 0.15){
        out.ind = grp1.ind
