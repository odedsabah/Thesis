...
class MicrobiomeAnalysis:
    ...
    def compute_beta_diversities(self, df_list, metric, TAXA_level, threshold):
        ...
        def max_diff(x):
            x_without_nan = x[~np.isnan(x)]
            if len(x_without_nan) == 0:
                return np.nan
            return np.max(x_without_nan) - np.min(x_without_nan)
        
        # Function to process and rename summary DataFrames
        def summarize_dataframe(df):
            summary_df = df.agg({
                'A': ['median', q1, q3, max_diff],
                'B': ['median', q1, q3, max_diff]
            })
            return summary_df 
        # Summarize DataFrames
        summary_distances_df = summarize_dataframe(distances_df)
        print(summary_distances_df)
    ...
    def compute_median_diversities(self, df_list, metric, TAXA_level,threshold):
        ...
        sns.heatmap(median_diversities_rounded, annot=annotations_with_newline, fmt='', cmap=sns.color_palette("icefire", as_cmap=True))
    ...
    def compute_alpha_diversities(self, df_list, TAXA_level, threshold):
        ...
        def summarize_dataframe(df):
            ...
            return summary_df 
        # Summarize DataFrames
        summary_df_shannon = summarize_dataframe(results_df_shannon)
        summary_df_chao1 = summarize_dataframe(results_df_chao1)
        summary_df_simpson = summarize_dataframe(results_df_simpson)
        ...
        plt.tight_layout()
        plt.show()
        return summary_df_shannon, summary_df_chao1, summary_df_simpson        
